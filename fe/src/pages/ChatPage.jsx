...
export default function ChatPage() {
    ...
    const handleCreateRoom = async (values) => {
        try {
            // values.members là array username hoặc id
            // Backend yêu cầu List<String> participantIds
            // Giả sử API /users trả về id, nếu không thì phải map username sang id
            // Ở đây mình giả định values.members chứa ID
            
            const payload = {
                name: values.name,
                participantIds: [...values.members, user.username] // Thêm chính mình vào nhóm (hoặc ID nếu dùng ID)
            };
        ...
        <MainLayout>
                ...
                <div className="chat-sidebar">
                    ...
                    <div style={{ overflowY: 'auto', flex: 1 }}>
                        {rooms.map(room => (
                            ...
                            >
                                <Avatar style={{ backgroundColor: room.isGroup ? '#87d068' : '#1890ff' }}>
                                    {room.name ? room.name.charAt(0).toUpperCase() : 'U'}
                                </Avatar>
                                <div style={{ overflow: 'hidden' }}>
                                    <div style={{ fontWeight: 600 }}>{room.name}</div>
                                    <div style={{ fontSize: 12, color: '#888', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>
                                        {room.isGroup ? 'Nhóm chat' : 'Tin nhắn riêng'}
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            <Modal title="Tạo cuộc trò chuyện mới" open={isModalOpen} onCancel={() => setIsModalOpen(false)} footer={null}>
                <Form form={form} onFinish={handleCreateRoom} layout="vertical">
                    <Form.Item name="name" label="Tên nhóm (Để trống nếu chat riêng)" rules={[{ required: true, message: 'Nhập tên nhóm!' }]}>
                        <Input placeholder="Ví dụ: Team Building..." />
                    </Form.Item>
                    
                    <Form.Item name="members" label="Thêm thành viên" rules={[{ required: true, message: 'Chọn ít nhất 1 người!' }]}>
                        <Select mode="multiple" placeholder="Chọn thành viên...">
                            {allUsers.map(u => (
                                <Select.Option key={u.id} value={u.username}>
                                    {u.username} ({u.email})
                                </Select.Option>
                            ))}
                        </Select>
